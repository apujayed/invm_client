...
const ViewList = () => {
  const { ExportCSVButton } = CSVExport;
    const {deleteLcpur,selectlcpurchase,serverUrl,lcPurchase,editItem,accountList,getAccounts,accounts,selectSell,deleteAcccount} = useContext(Context);

    const [deleteurl2, setDeleteurl] = useState(serverUrl+`/deletelcpurchase`);
    const [findurl, setFindurl] = useState(serverUrl+`/selectlcpurchase`)
        ...
        },
        {
          ...
          formatter: (rowContent, row) => {
            return (
              ...
              <div>
                 <Button variant="info"  onClick={() => selectlcpurchase(row.inv_id,findurl)} >Edit</Button>{' '}
